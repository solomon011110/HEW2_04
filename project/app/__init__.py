...
✔︎家族や友人と気軽に楽しみたい！""")
        ...
        if not Product.query.filter_by(name='ワケありだけど、味は本格派！「もったいないポテトチップス」').first():  
            apple = Product(name='ワケありだけど、味は本格派！「もったいないポテトチップス」', sale_price=500, category="デザート", defect_reason = """ちょっと形が不揃いだったり、サイズが小さかったりして規格外になったジャガイモを、美味しく有効活用！素材の味を活かしたポテトチップスに仕上げました。カリッとした食感とジャガイモの自然な甘み・旨みがたまらない、シンプルながら奥深い味わいです。
...
""")
        ...
        if not Product.query.filter_by(name='見た目はちょっと不揃い！？「もったいないかまぼこ」').first():  
            apple = Product(name='見た目はちょっと不揃い！？「もったいないかまぼこ」', sale_price=500, category="デザート", defect_reason = """形が少し歪んでいたり、端っこが多かったりして規格外になったかまぼこを、美味しく有効活用！厳選した魚のすり身を使用し、ふんわりしながらもしっかり弾力のある食感に仕上げました。見た目はちょっとワイルドですが、味はいつものかまぼこと同じです！
...
""")
    ...
    if not Product.query.filter_by(name='見た目バラバラ？！「もったいない野菜チップス」').first():  
            apple = Product(name='見た目バラバラ？！「もったいない野菜チップス」', sale_price=500, category="デザート", defect_reason = """ちょっと形が不揃いだったり、小さすぎたりして規格外になった野菜を、美味しくチップスに仕上げました！素材の味を活かし、添加物不使用＆ノンフライ製法でパリッと軽い食感に。野菜本来の甘みと旨みをぎゅっと凝縮しています。
...
""")
    ...
    if not Product.query.filter_by(name='ちょっとワケあり？！「もったいないフルーツジャム」').first():  
            apple = Product(name='ちょっとワケあり？！「もったいないフルーツジャム」', sale_price=500, category="デザート", defect_reason = """形が不揃いだったり、小さすぎたりして市場に出せなかったフルーツを、美味しいジャムにしました！味や香りは一級品なのに、見た目の理由だけで規格外になった果物たちを無駄なく活用。砂糖控えめ・無添加製法で、果実本来の甘みと酸味をぎゅっと凝縮しています。
...
""")
    ...
    if not Product.query.filter_by(name='見た目がちょっとワケあり？！「もったいないトマト缶」').first():  
            apple = Product(name='見た目がちょっとワケあり？！「もったいないトマト缶」', sale_price=500, category="デザート", defect_reason = """サイズがバラバラだったり、形が少し歪んでいたりして市場に出せなかった完熟トマトを、美味しいトマト缶にしました！味や栄養価は正規品と変わらず、イタリア産・国産の厳選トマトを100%使用。添加物は一切使わず、トマトの旨みをそのまま閉じ込めています。
...
""")
    ...
    if not Product.query.filter_by(name='見た目はワケあり！？「もったいない野菜ジュース」').first():  
            apple = Product(name='見た目はワケあり！？「もったいない野菜ジュース」', sale_price=500, category="デザート", defect_reason = """形がちょっと歪んでいたり、大きさがバラバラだったりして市場に出せなかった規格外の野菜を、栄養たっぷりのジュースにしました！厳選したトマト、ニンジン、ホウレンソウ、セロリ、カボチャなどを使用し、砂糖・保存料・着色料は一切不使用。野菜本来の甘みと旨みをぎゅっと凝縮しています。
...
""")
    ...
    if not Product.query.filter_by(name='ちょっとワケあり！？「もったいないピーナッツバター」').first():  
            apple = Product(name='ちょっとワケあり！？「もったいないピーナッツバター」', sale_price=500, category="デザート", defect_reason = """サイズが小さかったり、形が不揃いだったりして市場に出せなかった規格外ピーナッツを100％使用！余計な添加物を使わず、砂糖・保存料不使用でピーナッツ本来の香ばしさと濃厚なコクをそのまま楽しめるピーナッツバターに仕上げました。
...
""")
    ...
    if not Product.query.filter_by(name='見た目はちょっとワケあり？！「もったいなたくあん」').first():  
            apple = Product(name='見た目はちょっとワケあり？！「もったいなたくあん」', sale_price=500, category="デザート", defect_reason = """""")
            
            db.session.add(apple)

    if not Product.query.filter_by(name='ワケありだけど、味は本格派！「もったいないポテトチップス」').first():  
            apple = Product(name='ちょっとワケあり？！濃厚モンブランケーキ', sale_price=500, category="デザート", defect_reason = """形が不揃いだったり、皮が少し傷ついていたりして市場に出せなかった大根を使用した、お得で美味しいたくあんです。手間暇かけて、昔ながらの製法でじっくり漬け込み、無添加・自然発酵で仕上げました。
